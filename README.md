...
- Codex 桌面版或已安装并可执行的 Codex CLI。交互式“继续/新建任务”至少需要其中一种。
- 推荐同时安装 Codex CLI。只安装桌面版时，ThreadShelf 会只读导入本地 JSONL；CLI 的 `app-server` 可避免直接读取活动会话文件，并启用归档、取消归档和标题重命名。
...
Codex 桌面版和 Codex CLI 是两个独立的安装与探测入口。即使桌面版已经可以正常使用，也建议按 [Codex CLI 官方文档](https://learn.chatgpt.com/docs/codex/cli) 安装 CLI。已安装 Node.js 时可运行：
...
安装后重启 ThreadShelf。它会优先使用 `codex app-server`；如果仍未找到 CLI，可设置 `THREADSHELF_CODEX_CLI` 指向 `codex.exe`、`codex.cmd` 或 `codex.bat`。
...
1. 启动 ThreadShelf。它会优先尝试 `codex app-server`，失败时自动使用本地 JSONL 只读导入。只安装 Codex 桌面版并不代表 CLI 可用。
...
本地 fallback 会以共享只读方式打开 JSONL，不会阻止 Codex 桌面版继续写入。如果活动会话文件仍被短暂独占，ThreadShelf 会显示可重试的错误页；稍后重试，或安装 CLI 改用 `app-server`。
...
这通常发生在只安装 Codex 桌面版、ThreadShelf 正通过本地 JSONL fallback 加载，而桌面版恰好在写入活动会话时。新版会使用共享只读模式；如果文件仍被短暂独占，请在错误页点击“重试”。推荐安装 Codex CLI 并确认 `codex --version` 可运行，ThreadShelf 随后会优先通过 `app-server` 加载。
